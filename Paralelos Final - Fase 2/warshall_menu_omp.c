...
//
// Compilar (Linux):
//   gcc -O3 -std=c11 -fopenmp warshall_menu_omp.c -o warshall_omp
// (si tu Linux requiere):
//   gcc -O3 -std=c11 -fopenmp warshall_menu_omp.c -o warshall_omp -lrt
//
// Ejecutar:
//   ./warshall_omp
//   ./warshall_omp 1024 0.05 1234 3 0 0
//
// Nota:
// - Paralelizamos el bucle de i (filas) para cada k.
// - Cada hilo escribe solo su fila row_i, no hay condiciones de carrera.
// - k queda secuencial (dependencia entre iteraciones).
...
void warshall_logical_omp(uint8_t* A, int N) {
    ...
    // k debe ser secuencial, pero paralelizamos i (cada hilo escribe su fila).
    for (int k = 0; k < N; k++) {
...
// =====================================================
// Ejecutar en modo verify/timing
// =====================================================
